...
#define CS_HIGH()	SDCard_CS_SetHigh()	/* MMC CS = H */
#define MMC_CD		(SDCard_CD_GetValue()==0) 	/* Card detected   (yes:true, no:false, default:true) */
#define MMC_WP		(false)		/* Write protected (yes:true, no:false, default:false) */

/* SPI bit rate controls */
#define	FCLK_SLOW()	setSlowSPI()		/* Set slow clock for card initialization (100k-400k) */
#define	FCLK_FAST()	setFastSPI() 		/* Set fast clock for generic read/write */

/* SPI read/write function */
#define xchg_spi        spi1_exchangeByte
#define xmit_spi_multi  spi1_writeBlock
#define rcvr_spi_multi  spi1_readBlock
...
static void setFastSPI() {
    spi1_close();
    spi1_open( SDFAST_CONFIG );
    usefastspi= 1;
}

static void setSlowSPI() {
    spi1_close();
    spi1_open( SDSLOW_CONFIG );
    usefastspi= 0;
}
...
)
{
	...
	if (token != 0xFD) {	/* Not StopTran token */
		xmit_spi_multi((void *)buff, 512);	/* Xmit the data block to the MMC */
